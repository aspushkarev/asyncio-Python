...
async def main():
    print(f"Начало цепочки асинхронных вызовов")
    x = 1
    result = await asyncio.create_task(first_function(x))
    result = await asyncio.create_task(second_function(result))
    result = await asyncio.create_task(third_function(result))
    final_result = await asyncio.create_task(fourth_function(result))
